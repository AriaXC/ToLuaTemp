...
function Json2.encode (v)
  ...
  if vtype=='table' then
    ...
    if bArray then
      for i = 1,maxCount do
        table.insert(rval, Json2.encode(v[i]))
      end
    else	-- An object, not an array
      for i,j in base.pairs(v) do
        if isEncodable(i) and isEncodable(j) then
          table.insert(rval, '"' .. encodeString(i) .. '":' .. Json2.encode(j))
        end
      end
    end
    if bArray then
...
function Json2.decode(s, startPos)
  ...
  if string.sub(s,startPos,startPos+1)=='/*' then
    return Json2.decode(s, decode_scanComment(s,startPos))
  end
  -- Otherwise, it must be a constant
  return Json2.decode_scanConstant(s,startPos)
end
